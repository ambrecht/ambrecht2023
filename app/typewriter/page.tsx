...
export default function TypewriterPage() {
  ...
  return (
    <div
      ref={containerRef}
      className={`min-h-screen flex flex-col ${isFullscreen ? "bg-[#f8f5f0]" : "bg-[#f3efe9]"} text-gray-900 transition-colors duration-300`}
    >
      <header
        className={`border-b ${isFullscreen ? "border-[#e0dcd3]" : "border-[#d3d0cb]"} transition-colors duration-300`}
      >
        {!isFullscreen ? (
          <ControlBar
            wordCount={wordCount}
      ...
      <main className="flex-1 flex flex-col p-4 md:p-6 lg:p-8">
        <section
          className={`flex-1 flex flex-col ${isFullscreen ? "bg-white" : "bg-[#fcfcfa]"} shadow-md rounded-lg overflow-hidden transition-colors duration-300`}
        >
          <WritingArea
            lines={lines}
