...
// Aktualisiere die calculateOptimalLineLength-Funktion, um robuster zu sein
/**
 * Berechnet die optimale Zeilenlänge basierend auf der Containerbreite
 *
 * @param containerWidth - Die Breite des Containers in Pixeln
 * @param fontSize - Die Schriftgröße in Pixeln
...
export function calculateOptimalLineLength(containerWidth: number, fontSize: number): number {
  // Sicherheitsprüfungen für ungültige Eingaben
  if (!containerWidth || containerWidth <= 0) containerWidth = 800
  if (!fontSize || fontSize <= 0) fontSize = 24

  // Durchschnittliche Zeichenbreite in Pixeln (abhängig von der Schriftart und -größe)
  // Für eine Serif-Schrift mit der angegebenen Größe
  const avgCharWidth = fontSize * 0.6

  // Berücksichtige Padding (links und rechts)
  const padding = 48 // 24px Padding auf jeder Seite
  const availableWidth = containerWidth > padding ? containerWidth - padding : containerWidth * 0.8

  // Berechne die maximale Anzahl von Zeichen, die in die Zeile passen
  let maxChars = Math.floor(availableWidth / avgCharWidth)

  // Begrenze auf maximal 200 Zeichen (wie in der neuen Anforderung)
  maxChars = Math.min(maxChars, 200)

  // Stelle sicher, dass mindestens 10 Zeichen pro Zeile angezeigt werden (neue Anforderung)
